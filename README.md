...
- 🧊 **3D 依真實比例生成**：選一個管件 → 依真實 mm 建立立體模型，拖曳旋轉、雙指/滾輪縮放；**尺寸線與角度弧直接畫在模型上**（技法同「螺牙查詢」）
- 📏 **尺寸都在圖上**：對焊管件（彎頭/三通/大小頭/管帽）標 ASME B16.9 實際值，隨口徑切換即時更新
- 📖 **說明欄**：每個管件的一般叫法（台灣水電俗稱）、用途、常見材質、規範、全口徑尺寸表
- 📋 **管徑對照表**：台制「分」× 英吋 × A 稱呼 × SGP 外徑/厚度/重量 × ASME 外徑
- **零依賴**：純 HTML／JS，3D 為自寫 canvas 軟體渲染，離線也能跑
